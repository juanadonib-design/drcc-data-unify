...
def solo_numeros(key):
    ...
    st.session_state[key] = re.sub(r"\D", "", str(valor))

st.set_page_config(page_title="DRCC DATA UNIFY", page_icon="📊", layout="wide")
...
st.markdown('<p style="color:#555; font-size:16px;">Ahorra tiempo al unificar estructuras programáticas y libramientos en SIGEF.</p>', unsafe_allow_html=True)
st.divider()

modo = st.radio("🧭 Selecciona el modo de trabajo", ["🔁 Modo múltiple (Excel)", "🧩 Modo manual (uno por uno)"], horizontal=True)
st.divider()
    ...
    with col1:
        ...
        if uploaded_file:
            try:
                ...
                tiene_texto = any(any(c.isalpha() for c in str(valor)) for valor in posible_header)
        ...
        if df is None:
            ...
            try:
                ...
                if override:
                    ...
                    col_estructura = st.selectbox("Selecciona la columna de Estructura Programática", columnas)
                    col_libramiento = st.selectbox("Selecciona la columna de Número de Libramiento", columnas)
                else:
...
if modo.startswith("🧩"):
    ...
    with col1:
        st.text_input("Estructura Programática (12 dígitos)", key="estructura", on_change=solo_numeros, args=("estructura",))

    with col2:
        st.text_input("Número de Libramiento (1 o 5 dígitos)", key="libramiento", on_change=solo_numeros, args=("libramiento",))

    estructura = st.session_state.get("estructura", "")
    libramiento = st.session_state.get("libramiento", "")

    if estructura and libramiento:
        if len(estructura) == 12 and 1 <= len(libramiento) <= 5:
            resultado = f"{estructura[:4]}.{estructura[4:6]}.{estructura[8:]}.{libramiento}"
